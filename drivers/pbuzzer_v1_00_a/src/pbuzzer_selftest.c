//////////////////////////////////////////////////////////////////////////////
// Filename:          D:\Xtemp\hrm506_EMB\Lab56/drivers/pbuzzer_v1_00_a/src/pbuzzer_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the pbuzzer driver
...
XStatus PBUZZER_SelfTest(void * baseaddr_p)
{
  ...
  PBUZZER_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = PBUZZER_mReadSlaveReg0(baseaddr, 0);
  ...
  PBUZZER_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = PBUZZER_mReadSlaveReg1(baseaddr, 0);
  ...
  Reg32Value = PBUZZER_mReadReg(baseaddr, PBUZZER_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  PBUZZER_mWriteReg(baseaddr, PBUZZER_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = PBUZZER_mReadReg(baseaddr, PBUZZER_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  PBUZZER_mWriteReg(baseaddr, PBUZZER_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  PBUZZER_EnableInterrupt(baseaddr_p);
