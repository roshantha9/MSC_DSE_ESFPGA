//////////////////////////////////////////////////////////////////////////////
// Filename:          D:\Xtemp\hrm506_EMB\Lab56/drivers/sec_timer_gated_v1_00_a/src/sec_timer_gated_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the sec_timer_gated driver
...
XStatus SEC_TIMER_GATED_SelfTest(void * baseaddr_p)
{
  ...
  SEC_TIMER_GATED_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = SEC_TIMER_GATED_mReadSlaveReg0(baseaddr, 0);
  ...
  SEC_TIMER_GATED_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = SEC_TIMER_GATED_mReadSlaveReg1(baseaddr, 0);
  ...
  Reg32Value = SEC_TIMER_GATED_mReadReg(baseaddr, SEC_TIMER_GATED_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  SEC_TIMER_GATED_mWriteReg(baseaddr, SEC_TIMER_GATED_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = SEC_TIMER_GATED_mReadReg(baseaddr, SEC_TIMER_GATED_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  SEC_TIMER_GATED_mWriteReg(baseaddr, SEC_TIMER_GATED_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  SEC_TIMER_GATED_EnableInterrupt(baseaddr_p);
