//////////////////////////////////////////////////////////////////////////////
// Filename:          D:\Xtemp\hrm506_EMB\Lab56/drivers/sec_timer_v1_00_a/src/sec_timer_selftest.c
// Version:           1.00.a
// Description:       Contains a diagnostic self-test function for the sec_timer driver
...
XStatus SEC_TIMER_SelfTest(void * baseaddr_p)
{
  ...
  SEC_TIMER_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = SEC_TIMER_mReadSlaveReg0(baseaddr, 0);
  ...
  SEC_TIMER_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = SEC_TIMER_mReadSlaveReg1(baseaddr, 0);
  ...
  Reg32Value = SEC_TIMER_mReadReg(baseaddr, SEC_TIMER_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  SEC_TIMER_mWriteReg(baseaddr, SEC_TIMER_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = SEC_TIMER_mReadReg(baseaddr, SEC_TIMER_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  SEC_TIMER_mWriteReg(baseaddr, SEC_TIMER_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  SEC_TIMER_EnableInterrupt(baseaddr_p);
